...
//3. unsuccessful, invalid invoiceId as invoice is in trash
...
describe("Testing route DELETE /invoices/{invoiceIds}/trash", function() {
    it("Delete from trash Unit Tests", async function() {
        ...
        await request(app)
            .delete(`/invoices/${[invoice1.body.invoiceId]}/trash`)
            .set("token", user2.body.token)
            .expect(403)
            .expect("Content-Type", /application\/json/)
        ...
        // Invoice list for user1 should have 2 invoices
        const invoiceList1 = await request(app)
            .get("/invoicesV2")
            .set("token", user1.body.token)
            .expect(200);

        assert.strictEqual(invoiceList1.body.success, true);
        assert.strictEqual(invoiceList1.body.invoices[0].invoiceid, invoice1.body.invoiceId);
        assert.strictEqual(invoiceList1.body.invoices[0].invoicename, "Invoice 1");
        assert.strictEqual(invoiceList1.body.invoices[0].amount, 123.45);
        assert.strictEqual(invoiceList1.body.invoices[1].invoiceid, invoice2.body.invoiceId);
        assert.strictEqual(invoiceList1.body.invoices[1].invoicename, "Invoice 2");
        assert.strictEqual(invoiceList1.body.invoices[1].amount, 543.21);
        ...
        // unsuccessful, invoiceid exists but is in trash
        await request(app)
            .delete(`/invoices/${[invoice1.body.invoiceId]}/trash`)
            .set("token", user1.body.token)
            .expect(400)
            .expect("Content-Type", /application\/json/)
            .expect({"success": false, "error": `${invoice1.body.invoiceId} refers to an invoice in trash`});

        // Invoice list for user1 should be empty
        await request(app)
            .get("/invoicesV2")
            .set("token", user1.body.token)
            .expect(200)
        ...
        // Invoice list for user2 should still have invoices
        let invoiceList2 = await request(app)
            .get("/invoicesV2")
            .set("token", user2.body.token)
            .expect(200);

        assert.strictEqual(invoiceList2.body.success, true);
        assert.strictEqual(invoiceList2.body.invoices[0].invoiceid, invoice3.body.invoiceId);
        assert.strictEqual(invoiceList2.body.invoices[0].invoicename, "Invoice 1");
        assert.strictEqual(invoiceList2.body.invoices[0].amount, 123.45);
        assert.strictEqual(invoiceList2.body.invoices[1].invoiceid, invoice4.body.invoiceId);
        assert.strictEqual(invoiceList2.body.invoices[1].invoicename, "Invoice 2");
        assert.strictEqual(invoiceList2.body.invoices[1].amount, 543.21);
        assert.strictEqual(invoiceList2.body.invoices[2].invoiceid, invoice5.body.invoiceId);
        assert.strictEqual(invoiceList2.body.invoices[2].invoicename, "Invoice 3");
        assert.strictEqual(invoiceList2.body.invoices[2].amount, 999.99);

        // Trash some invoices but not all for user2
        await request(app)
            .delete(`/invoices/${[invoice3.body.invoiceId, invoice4.body.invoiceId]}/trash`)
            .set("token", user2.body.token)
            .expect(200)
            .expect("Content-Type", /application\/json/)
            .expect({"success": true});

        // Invoice list for user2 should still have 1 invoice
        invoiceList2 = await request(app)
            .get("/invoicesV2")
            .set("token", user2.body.token)
            .expect(200);

        assert.strictEqual(invoiceList2.body.success, true);
        assert.strictEqual(invoiceList2.body.invoices[0].invoiceid, invoice5.body.invoiceId);
        assert.strictEqual(invoiceList2.body.invoices[0].invoicename, "Invoice 3");
        assert.strictEqual(invoiceList2.body.invoices[0].amount, 999.99);
    });
});
